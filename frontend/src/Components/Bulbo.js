...
export default class Bulbo extends Component {
  constructor(props) {
    super(props);
    this.state = {
      productosB:[],
  ...
  async componentDidMount(){
   const res = await axios.get('http://localhost:3001/api/Productos/Bulboss/');
    this.setState({productosB: res.data}); 
  }
      ...
      <div class="container row" style={{minWidth:'108%',minHeight:"670px",paddingLeft:"50px",backgroundSize:'contain',backgroundImage:"url(https://ak.picdn.net/shutterstock/videos/18869951/thumb/1.jpg)",backgroundColor:'rgb(100,100,100',backgroundBlendMode:'soft-light'}}>
        <div className="col-md-12" style={{padding: "25px 0"}}>
          <ol style={{background:"rgb(92 90 90)",borderRadius:"10px"}}  className="col-md-5 breadcrumb">
            ...
            <li >
              <a style={{color: "antiquewhite"}} href='/Plantas_de_Exterior'>  
                <i style={{color: "white",fontSize:"1.20em"}} class="faExteriorMigas ">
            ...
            <li>
              <i style={{color: "antiquewhite",fontSize:"1.20em"}} class="RosalMigas"> Bulbos </i>
            </li>
          </ol>
        </div>    
        <div id='mySection' className='row' style={{minWidth:"90%"}}>
        {this.state.productosB.length ===0 ? <div style={{color:'white'}}><h1>Este apartado no cuenta con productos por el momento</h1></div>:<Fragment>
   
        {this.state.productosB.map(producto => (
